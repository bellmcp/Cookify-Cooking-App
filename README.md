...
* 1. Download the complete package by hit the green button saying "Clone or download" then choose the "Download ZIP" option.
* 2. Download and install [Node.js (LTS version)](https://nodejs.org/en/). Check if the installation is correct by run `node -v` and `npm -v` commands in Terminal.
* 3. Access to the main project's folder path by `cd` command. Check if the current path location is correct by `ls` command, you will see "dist", "package-lock.json", "src", "package.json" and "webpack.config.js" as the results.
* 4. Download and install essential plugins by run `npm install` command. Check if the installation is correct, you will see the "node_modules" folder in the main project folder. 
* 5. Run the project in browser by simply type `npm run start` command.
* 6. Terminate the project by hitting `Ctrl + C` in the Terminal.
